    ...
    async function run(arguments) {
      ...
      try {
        ...
        const result = await connection.execute(
            ...
            NATURAL JOIN
            (
            SELECT EXTRACT(MONTH FROM DAY) AS X2, ROUND(100 *SUM(NEW_CASES)/ (SELECT POPULATION FROM "PROJECT_COUNTRY" WHERE NAME = 'INDIA'), 8) AS Y4
            FROM "PROJECT_STATISTIC2020"
            WHERE COUNTRY_NAME = 'INDIA'
            GROUP  BY  EXTRACT(MONTH FROM DAY)
            ORDER BY EXTRACT(MONTH FROM DAY)
